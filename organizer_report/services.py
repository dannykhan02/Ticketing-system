...
class ReportDataProcessor:
    ...
    def process_report_data(report_data: Dict[str, Any], event_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        integrity_check = detailed_stats.get('integrity_issues', [])
        processed_data = {
            'total_tickets_sold': total_tickets_sold,
...
class ReportService:
    ...
    def create_report_data(self, event_id: int, start_date: datetime, end_date: datetime, ticket_type_id: Optional[int] = None, target_currency_code: Optional[str] = None) -> Dict[str, Any]:
        ...
        total_revenue_display = self.currency_converter.convert_amount(total_revenue_base, base_currency_code, display_currency_code)
        revenue_by_ticket_type = {ticket_type: float(self.currency_converter.convert_amount(revenue, base_currency_code, display_currency_code)) for ticket_type, revenue in revenue_data}
        ...
        if base_currency_code != display_currency_code:
            ...
            report_data['conversion_rate_used'] = float(self.currency_converter.convert_amount(Decimal('1'), base_currency_code, display_currency_code))
        if ticket_type_id:
    ...
    def generate_complete_report(self, event_id: int, organizer_id: int, start_date: datetime, end_date: datetime, session, ticket_type_id: Optional[int] = None, target_currency_code: Optional[str] = None, send_email: bool = False, recipient_email: str = None) -> Dict[str, Any]:
        try:
            report_data = self.create_report_data(event_id, start_date, end_date, ticket_type_id, target_currency_code)
            ...
            pdf_path = self.pdf_generator.generate_pdf(report_data=report_data, chart_paths=chart_paths, output_path=pdf_path, session=session, event_id=event_id, target_currency=target_currency_code or "KES")
            csv_path = CSVReportGenerator.generate_csv(report_data=report_data, output_path=csv_path, session=session, event_id=event_id)
            email_sent = False
            if send_email and recipient_email and self.config.include_email:
                email_sent = self.send_report_email(report_data, pdf_path, csv_path, recipient_email)
            return {
                'success': True,
    ...
    def send_report_email(self, report_data: Dict[str, Any], pdf_path: str, csv_path: str, recipient_email: str) -> bool:
        try:
    ...
    def _send_report_email(self, report_data: Dict[str, Any], pdf_path: str, csv_path: str, recipient_email: str) -> bool:
