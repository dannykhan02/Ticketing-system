...
# Define specific colors for each ticket type, matching frontend (use uppercase keys)
COLORS_BY_TICKET_PYTHON = {
    'REGULAR': colors.HexColor('#FF8042'),  # Orange
    'VIP': colors.HexColor('#FFBB28'),  # Yellow
    'STUDENT': colors.HexColor('#0088FE'),  # Blue
    'GROUP_OF_5': colors.HexColor('#00C49F'),  # Green
    'COUPLES': colors.HexColor('#FF6699'),  # Pinkish
    'EARLY_BIRD': colors.HexColor('#AA336A'),  # Purple
    'VVIP': colors.HexColor('#00FF00'),  # Bright Green for VVIP
    'GIVEAWAY': colors.HexColor('#CCCCCC'),  # Grey
    'UNKNOWN_TYPE': colors.HexColor('#A9A9A9'),  # Darker Grey for unknown types
}

# Fallback color if a ticket type is not in COLORS_BY_TICKET_PYTHON
FALLBACK_COLOR_PYTHON = COLORS_BY_TICKET_PYTHON.get('UNKNOWN_TYPE', colors.HexColor('#808080'))
