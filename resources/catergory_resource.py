...
class CategoryDetailResource(Resource):
    ...
    def put(self, category_id):
        """Update category (Admin only) with AI validation"""
        current_user = User.query.get(get_jwt_identity())
        if not current_user or current_user.role != UserRole.ADMIN:
        ...
        try:
            ...
            if use_ai:
                ...
                if validation['warnings'] or validation['suggestions']:
                    # Return warnings/suggestions for confirmation
                    if request.args.get('confirm', 'false').lower() != 'true':
                        return {
            ...
            elif use_ai and 'enhance_description' in data and data['enhance_description']:
